...
def download_detail():
    ...
    print(
        f"Existed : {len(tmdb_id2detail.keys())}\nLeft: {len(remain_movie_ids)}\n Start downloading."
    )
...
def download_credits(tmdb_id2detail):
    target_tmdb_id_list = [
        ...
        if json.loads(v)["budget"] > 0 and json.loads(v)["revenue"] > 0
    ]

    data_path = "tmdb_resources/tmdb_id2credit.json"
    tmdb_id2credit = (
        json.load(open(data_path, "r")) if os.path.exists(data_path) else {}
    )
    remain_movie_ids = list(set(target_tmdb_id_list) - set(tmdb_id2credit.keys()))

    print(
        f"Existed : {len(tmdb_id2credit.keys())}\nLeft: {len(remain_movie_ids)}\n Start downloading."
    )
    random.shuffle(remain_movie_ids)
    lock = threading.Lock()
    threads = []
